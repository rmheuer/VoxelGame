...
import com.github.rmheuer.voxel.client.Player;
...
public final class MultiplayerClientTest extends BaseGame {
...
//    private LevelRenderData levelRenderData;
    ...
    public MultiplayerClientTest() throws IOException {
        ...
        beginConnecting("localhost", 25565);
    }

    private void beginConnecting(String host, int port) {
        ...
        connectFuture.addListener((future) -> {
            if (future.isSuccess()) {
                ...
                runOnMainThread(() -> {
                    ...
                    networkHandler = new NetworkHandler(this, connection);
                });
            } else {
    ...
    public void resetPlayer(float x, float y, float z, float pitch, float yaw) {
        player = new Player(x, y, z, pitch, yaw);
    }
...
//                    setUI(new PauseMenuUI(this));
    ...
    private void fixedTick(float dt) {
        ...
        player.tickMovement(level.getBlockMap(), inputForward, inputRight, jump);
    ...
    public static void main(String[] args) {
        MultiplayerClientTest game;
        try {
            game = new MultiplayerClientTest();
        } catch (IOException e) {
